...
| F10 | S4 | `semble` index times out at 120s on big repos (incl. hermes-agent) → search unavailable there. | **FIXED (S4, calibrated twice)** — two overclaims corrected here. B5's "FIXED" was false, and the first S4 cap (60,000 source files) could never trip; the second (8,000 source files) was ALSO wrong — it over-refused (`/srv/fluxcp`, 4,251 source files, actually indexes in **4.4s**). ROOT CAUSE: the counter counted a different population than Semble walks. MEASURED (all-files vs Semble's own report, within ~2%): rathena-AI-world 251,378/~256,131 → FAILED 246s; hermes-agent 97,573/~99,299 → FAILED 144s; PerhapsAnotherWay-server 116,162 → FAILED >104s; fluxcp 67,168 → **OK 4.4s**; raw-mobile-client 8,528 → OK 9.1s. Source-extension counts are INVERTED relative to cost (fluxcp 4,251 OK vs PAP-server 4,157 FAIL). Guard now uses the unfiltered count with an 90,000 cap, which sits above every tree measured to succeed and below every tree measured to fail. VERIFIED: fluxcp allow→7.9s, hermes-ultimate-coding allow→0.4s, raw-mobile-client allow→0.2s; PAP-server / rathena-AI-world / hermes-agent all refused INSTANTLY (0.1–0.2s pre-flight) instead of burning the timeout. |
