...
| F10 | S4 | `semble` index times out at 120s on big repos (incl. hermes-agent) → search unavailable there. | **FIXED (S4, calibrated)** — the earlier "FIXED (B5)" was an OVERCLAIM and the first S4 attempt was *also* wrong. MEASURED: `hermes-agent` counts **12,329** files by the plugin's own preflight counter but **~99,299** by Semble's walker (8x — the counter applies different ignore rules), and indexing it **FAILED after 144.0s** against a 120s budget. The old cap of 60,000 could therefore never trip for the tree it was written to protect. Cap recalibrated to 8,000 from that measurement; message corrected too (it blamed "a home directory" for a non-home tree). Verified: big tree now refused in **0.81s** (was 144s of doomed work), real project still indexes in **1.75s**. |
