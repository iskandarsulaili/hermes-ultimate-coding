...
| F10 | S4 | `semble` index times out at 120s on big repos (incl. hermes-agent) → search unavailable there. | **PARTIALLY FIXED (S4)** — corrected: the earlier "FIXED (B5)" was an OVERCLAIM. B5 only added a refusal for `$HOME`; indexing `hermes-agent` (12,329 indexable files, under the 60,000-file cap) still times out at 120s and reports the timeout. The file-count guard is unrelated to the timeout — the cap does not predict it. Honest state: big-repo indexing needs a raised `HERMES_SEMBLE_INDEX_TIMEOUT` or a `.sembleignore`; the pre-flight guard prevents the orphan-thread case only. |
