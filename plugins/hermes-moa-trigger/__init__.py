    ...
    ctx.register_tool(
        ...
        schema={
            ...
            "description": (
                ...
                "task state. Call this at PLANNING MOMENTS mid-task: before a "
                "large or irreversible change (refactor, migration, rewrite), "
                "after a test/command failure that may invalidate the current "
                "approach, when the plan must change based on new information, "
                "or when the task is complex and a deep re-think would de-risk "
                "the next step. Runs the MoA max-reasoning advisor on the live "
                "conversation and returns its advice — read it and apply it "
                "before continuing. (Note: todo-plan writes already trigger "
                "this automatically via middleware; use this tool for other "
                "planning moments.)"
            ),
            "parameters": {
                "type": "object",
