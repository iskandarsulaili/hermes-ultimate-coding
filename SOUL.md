...
- **hermes-anchored** (2 tools): `dev_tool_search`, `anchored_status`. Anchored Standard — narrow the first model request to a minimal tool catalog (anchors the reasoning trajectory), then promote to a resident set after the first request. On-demand tool unlock. Enabled by default; `/anchored disable` to opt out (MOA disabled by default).
